...
{
    private $menu = 'Home';
    private $idx_base_url = 'ebook/ajaxPaginationData';
    private $model_name = 'ebook_model';
    private $page_topic = 'ebook';
    private $controller = 'main/ebook';
    private $views_folder_group = 'ebook/';
    private $user_id = '';
    private $key_edit = 'id_ebook';
    private $search_place_holder = 'Untuk pencarian data, ketik Judul, lalu tekan enter';
    private $col_title = array('Judul Ebook', 'Karya', 'File');
    private $list_field = array('title', 'karya', 'file');
    private $page_title = 'Ebook';
    private $table_width = '100';
    private $tblname = 'tbl_ebook';
    ...
    public function index()
    {
        ...
        $custom_content[] =  array('custom_field' => 'file', 'content' => '<a target="_blank" href="' . base_url() . 'assets/images/file_ebook/', 'middle_content' => '>', 'last_content' => '</a>');
    ...
    {
        // $cbo_cat = create_chosen_db_combo('id_skill_matrix', 'id_kat_ebook', 'ref_skill_matrix', 'id_skill_matrix', 'nama_skill_matrix', 'id_skill_matrix', '', '', '', false);
        $data = array(
            'controller' => $this->controller,
            'form_input_title' => 'Input Data Ebook',
            'root' => $this->menu,
            // 'cbo_cat' => $cbo_cat,
    ...
    public function saveData()
    {
        ...
        if ($this->input->post($this->key_edit)) {
            ...
            $id_ebook = $this->input->post($this->key_edit);
            $result =  $this->$nm_model->updateData($dtarray, $id_ebook);
        } else {
        ...
        if ($result['error'] == 'false') {
            ...
            $config['upload_path']   = FCPATH . 'assets/images/file_ebook/';
            $config['allowed_types'] = 'pdf|gif|jpg|jpeg|png';
            $this->load->library('upload', $config);

            // $encrypted_name =  $converter->encode($id_ebook);
            ...
            $arr_document['id_ebook'] = $id_ebook;
            $existing_image = '';
            ...
            foreach ($arr_obj_file_upload as $key => $value) {
                if (!$this->upload->do_upload($key)) {
                    ...
                    $existing_image = get_info_by_id("tbl_ebook", $key, "id_ebook", $id_ebook);
                    $incomplete_doc[] = $value;
                    $arr_document[$key] = $existing_image;
                } else {
                    //if no error then save the data with image info included
                    // $source_path = FCPATH . 'assets/images/file_ebook/';
                    // $target_path = FCPATH . 'assets/images/file_ebook/';
                    $upload_data = $this->upload->data();
                    // $img_path    = FCPATH . 'assets/images/file_ebook/' . $upload_data['file_name'];
                    ...
                    $fname = $upload_data['file_name'] .  date("Y-m-d H-i-s") . "_" . $id_ebook;
            ...
            $update_data = $this->$nm_model->update_doc($arr_document,  $id_ebook);
    ...
    public function editData()
    {
        ...
        // $cbo_cat = create_chosen_db_combo('id_skill_matrix', 'id_kat_ebook', 'ref_skill_matrix', 'id_skill_matrix', 'nama_skill_matrix', 'id_skill_matrix', '', $init_data->id_kat_ebook, '', false);

        $data = array(
            'controller' => $this->controller,
            'key_edit' => $this->key_edit,
            'id_edit_name' => $this->key_edit,
            'form_input_title' => 'Edit Data Ebook',
            'root' => $this->menu,
            'dt_init' => $init_data,
            // 'cbo_cat' => $cbo_cat,
        );
    ...
    //     $dtarray = array(
    //         'title' => $this->input->post('title'),
    //         'id_kat_ebook' => $this->input->post('id_kat_ebook'),
