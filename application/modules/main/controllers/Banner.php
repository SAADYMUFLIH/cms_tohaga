...
{
    private $menu = 'Home';
    private $idx_base_url = 'banner/ajaxPaginationData';
    private $model_name = 'banner_model';
    private $page_topic = 'banner';
    private $controller = 'main/banner';
    private $views_folder_group = 'banner/';
    private $user_id = '';
    private $key_edit = 'id_banner';
    private $search_place_holder = 'Untuk pencarian data, ketik Judul, lalu tekan enter';
    private $col_title = array('Gambar');
    private $list_field = array('img');
    private $page_title = 'Banner';
    private $table_width = '100';
    private $tblname = 'tbl_banner';
    ...
    public function index()
    {
        ...
        $custom_content[] =  array('custom_field' => 'img', 'content' => '<img class="profile-user-img img-fluid" style="height: 90px;" src="' . base_url() . 'assets/images/banner/', 'img_content' => 'true');
    ...
    public function displayInputForm()
    {
        ...
        $data = array(
            'controller' => $this->controller,
            'form_input_title' => 'Input Data Banner',
            'root' => $this->menu,
    ...
    public function saveData()
    {
        ...
        if ($this->input->post($this->key_edit)) {
            ...
            $id_banner = $this->input->post($this->key_edit);
            $result =  $this->$nm_model->updateData($dtarray, $id_banner);
        } else {
        ...
        if ($result['error'] == 'false') {
            ...
            $config['upload_path']   = FCPATH . 'assets/images/banner/';
            $config['allowed_types'] = 'gif|jpg|jpeg|png';
            $this->load->library('upload', $config);

            $encrypted_name =  $converter->encode($id_banner);

            $arr_obj_file_upload = array(
                "img" => "Gambar Banner" //field di DB
            );
            $arr_document = array();
            $incomplete_doc = array();
            $arr_document['id_banner'] = $id_banner;
            $existing_image = '';
            ...
            foreach ($arr_obj_file_upload as $key => $value) {
                if (!$this->upload->do_upload($key)) {
                    ...
                    $existing_image = get_info_by_id("tbl_banner", $key, "id_banner", $id_banner);
                    $incomplete_doc[] = $value;
                    $arr_document[$key] = $existing_image;
                } else {
                    //if no error then save the data with image info included
                    $source_path = FCPATH . 'assets/images/banner/';
                    $target_path = FCPATH . 'assets/images/banner/';
                    $upload_data = $this->upload->data();
                    $img_path    = FCPATH . 'assets/images/banner/' . $upload_data['file_name'];
                    ...
                    $fname = $key . "_" .  date("Y-m-d H-i-s") . "_" . $id_banner;
            ...
            $update_data = $this->$nm_model->update_doc($arr_document,  $id_banner);
    ...
    public function editData()
    {
        ...
        $data = array(
            ...
            'key_edit' => $this->key_edit,
            'id_edit_name' => $this->key_edit,
            'form_input_title' => 'Edit Data Banner',
            'root' => $this->menu,
            'dt_init' => $init_data,
            // 'cbo_kat' => $cbo_kat,
            // 'cbo_anggota' => $cbo_anggota,
