...
{
    private $menu = 'Home';
    private $idx_base_url = 'posts/ajaxPaginationData';
    private $model_name = 'posts_model';
    private $page_topic = 'posts';
    private $controller = 'main/posts';
    private $views_folder_group = 'posts/';
    private $user_id = '';
    private $key_edit = 'id_posts';
    private $search_place_holder = 'Untuk pencarian data, ketik Judul, lalu tekan enter';
    private $col_title = array('Judul', 'Kategori', 'Tanggal Publish', 'Gambar');
    private $list_field = array('title', 'kat_posts', 'date_publish', 'img');
    private $page_title = 'Posts';
    ...
    {
        $cbo_kat = create_chosen_db_combo('id_kat_posts', 'id_kat_posts', 'sys_settings', 'id_setting', 'value_setting', 'id_setting', '', '', "where category_setting='post' AND value_setting != 'Knowledge'", false);
        $data = array(
            'controller' => $this->controller,
            'form_input_title' => 'Input Data Posts',
            'root' => $this->menu,
    ...
    {
        $nm_model = $this->model_name;
        $date_publish = str_replace("/","-", $this->input->post('datePublish'));
        // echo $date_publish;
        ...
        $dtarray = array(
            ...
            'id_kat_posts' => $this->input->post('id_kat_posts'),
        ...
        if ($this->input->post($this->key_edit)) {
            ...
            $clear = $this->$nm_model->cleartag($id_posts);
        } else {
        ...
        if( in_array( $this->input->post('id_kat_posts'),$allowtagid) ){
            if($this->input->post('taglist')){
                $tags = explode(',',$this->input->post('taglist'));
                for ($i=0; $i <count($tags) ; $i++) { 
                    $tag =  $tags[$i];
                    $tagdata = $this->$nm_model->gettagid($tag);
                    $inserttag = array(
                        'id_posts' =>$id_posts,
                        'id_tag' => $tagdata[0]['id_tag']
                    );
                    $this->$nm_model->savetags($inserttag);
                }
            }
        }
        //exit();
    ...
    public function editData()
    {
        ...
        $cbo_kat = create_chosen_db_combo('id_kat_posts', 'id_kat_posts', 'sys_settings', 'id_setting', 'value_setting', 'id_setting', '', $init_data->id_kat_posts, "where category_setting='post'", false);
        $taglist = $this->$nm_model->gettaglist($id);
        $data = array(
            'controller' => $this->controller,
            'key_edit' => $this->key_edit,
            'id_edit_name' => $this->key_edit,
            'form_input_title' => 'Edit Data Posts',
            'root' => $this->menu,
            'taglist' =>$taglist,
            'dt_init' => $init_data,
            'cbo_kat' => $cbo_kat,
