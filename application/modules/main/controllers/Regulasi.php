...
{
    private $menu = 'Home';
    private $idx_base_url = 'regulasi/ajaxPaginationData';
    private $model_name = 'regulasi_model';
    private $page_topic = 'regulasi';
    private $controller = 'main/regulasi';
    private $views_folder_group = 'regulasi/';
    private $user_id = '';
    private $key_edit = 'id_regulasi';
    private $search_place_holder = 'Untuk pencarian data, ketik Judul, lalu tekan enter';
    private $col_title = array('Judul Regulasi & Perizinan', 'Kategori', 'File');
    private $list_field = array('title', 'id_kat', 'img');
    private $page_title = 'Regulasi & Perizinan';
    private $table_width = '100';
    private $tblname = 'tbl_regulasi';
    ...
    public function index()
    {
        ...
        $custom_content[] =  array('custom_field' => 'img', 'content' => '<a target="_blank" href="' . base_url() . 'assets/images/file_regulasi/', 'middle_content' => '>', 'last_content' => '</a>');
    ...
    {
        // $cbo_cat = create_chosen_db_combo('id_skill_matrix', 'id_kat_regulasi', 'ref_skill_matrix', 'id_skill_matrix', 'nama_skill_matrix', 'id_skill_matrix', '', '', '', false);
        $data = array(
            'controller' => $this->controller,
            'form_input_title' => 'Input Data Regulasi & Perizinan',
            'root' => $this->menu,
            // 'cbo_cat' => $cbo_cat,
    ...
    public function saveData()
    {
        ...
        $dtarray = array(
            ...
            'id_kat_regulasi' => $this->input->post('id_kat_regulasi'),
        );
        ...
        if ($this->input->post($this->key_edit)) {
            ...
            $id_regulasi = $this->input->post($this->key_edit);
            $result =  $this->$nm_model->updateData($dtarray, $id_regulasi);
        } else {
            $dtarray['create_at'] = getsysdate();
            $dtarray['create_by'] = $this->user_id;
            $result =  $this->$nm_model->insertData($dtarray);
            $id_regulasi = $result['id'];
        }

        if ($result['error'] == 'false') {
            $converter = new Encryption;
            $config['upload_path']   = FCPATH . 'assets/images/file_regulasi/';
            $config['allowed_types'] = 'pdf';
            $this->load->library('upload', $config);

            // $encrypted_name =  $converter->encode($id_regulasi);

            $arr_obj_file_upload = array(
                "img" => "File Regulasi & Perizinan"
            );
            $arr_document = array();
            $incomplete_doc = array();
            $arr_document['id_regulasi'] = $id_regulasi;
            $existing_image = '';
            ...
            foreach ($arr_obj_file_upload as $key => $value) {
                if (!$this->upload->do_upload($key)) {
                    ...
                    $existing_image = get_info_by_id("tbl_regulasi", $key, "id_regulasi", $id_regulasi);
                    $incomplete_doc[] = $value;
                    $arr_document[$key] = $existing_image;
                } else {
                    //if no error then save the data with image info included
                    // $source_path = FCPATH . 'assets/images/file_regulasi/';
                    // $target_path = FCPATH . 'assets/images/file_regulasi/';
                    $upload_data = $this->upload->data();
                    // $img_path    = FCPATH . 'assets/images/file_regulasi/' . $upload_data['file_name'];
                    ...
                    $fname = $upload_data['file_name'] .  date("Y-m-d H-i-s") . "_" . $id_regulasi;
            ...
            $update_data = $this->$nm_model->update_doc($arr_document,  $id_regulasi);
    ...
    public function editData()
    {
        ...
        // $cbo_cat = create_chosen_db_combo('id_skill_matrix', 'id_kat_regulasi', 'ref_skill_matrix', 'id_skill_matrix', 'nama_skill_matrix', 'id_skill_matrix', '', $init_data->id_kat_regulasi, '', false);

        $data = array(
            'controller' => $this->controller,
            'key_edit' => $this->key_edit,
            'id_edit_name' => $this->key_edit,
            'form_input_title' => 'Edit Data Regulasi & Perizinan',
            'root' => $this->menu,
            'dt_init' => $init_data,
            // 'cbo_cat' => $cbo_cat,
        );
