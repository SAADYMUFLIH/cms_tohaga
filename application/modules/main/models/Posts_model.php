...
{
    private $selected_field = "posts.*, value_setting as kat_posts";
    private $tbl_name = "tbl_posts as posts";
    private $tbl_tags = "tbl_tag";
    private $selected_field_tags = "id_tag";
    private $tbl_name_for_insert = "tbl_posts";
    private $field_search = array("title");
    private $field_id = "id_posts";
    ...
    public function getCountRows($params = array())
    {
        ...
        $this->db->join("sys_settings as ss", "ss.id_setting = posts.id_kat_posts", "left");
        $this->db->where("category_setting='post' AND ss.id_setting != 6");
    ...
    public function remove_img($arr_dt, $id_key, $field)
    {
        ...
        if ($this->db->update($this->tbl_name, $arr_dt)) {
            if (file_exists('img_posts/' . $existing_image)) {
                unlink('img_posts/' . $existing_image);
    ...
    public function getDataForInit($id)
    {
        ...
        $this->db->from($this->tbl_name);
        $this->db->join("sys_settings as ss", "ss.id_setting = posts.id_kat_posts", "left");
        $this->db->where("category_setting='post'");
    ...
    public function exportToExcel($xls_data)
    {
        ...
        $arr_col = array("No", 'NIK', 'Nama Posts', 'No. BPJS', 'Tempat Lahir', 'Tgl Lahir', 'Jenis Kelamin', 'Kabupaten', 'Kecamatan', 'Kelurahan', 'Alamat', 'No. HP', 'Pendidikan', 'Pekerjaan', 'Status Kawin', 'Gol. Darah', 'Create At', 'Update At');
    ...
    public function cleartag($id_posts)
    {
        $query = $this->db->where("id_posts", $id_posts)
                            ->delete("tbl_posts_tags");
        return $query;
    }

    public function gettagid($name)
    {
        $this->db->select($this->selected_field_tags);
        $this->db->from($this->tbl_tags);
    ...
    public function savetags($arraytag)
    {
        if ($this->db->insert("tbl_posts_tags",$arraytag)) {
            $arr_response = array(
                'error' => 'false',
                'msg' => 'insert tag success'
            );
        } else {
            $error = $this->db->error();
            $msg_error = $error['message'];
    ...
    public function gettaglist($idposts)
    {   
        $this->db->select('tag.name');
        $this->db->from('tbl_posts_tags');
        $this->db->join("tbl_tag as tag", "tag.id_tag = tbl_posts_tags.id_tag", "left");
        $this->db->where("id_posts = '{$idposts}'");
        // $this->db->order_by('NAME', 'ASC');
        // $this->db->limit(1);
        $query = $this->db->get();
        // die($this->db->last_query());
