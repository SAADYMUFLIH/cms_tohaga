...
{
    private $selected_field = "umkm.*, su.full_name";
    private $tbl_name = "tbl_umkm as umkm";
    private $tbl_name_for_insert = "tbl_umkm";
    private $field_search = array("title");
    private $field_id = "id_umkm";
    ...
    public function getRows($params = array())
    {
        ...
        $this->db->from($this->tbl_name);
        $this->db->join("sys_users as su", "su.user_id = umkm.user_id", "right");
    ...
    public function remove_img($arr_dt, $id_key, $field)
    {
        ...
        if ($this->db->update($this->tbl_name, $arr_dt)) {
            if (file_exists('assets/images/file_umkm/' . $existing_image)) {
                unlink('assets/images/file_umkm/' . $existing_image);
    ...
    {
        $this->db->select($this->selected_field.", nama_provinsi, nama_kabupaten, nama_kecamatan, nama_kelurahan");
        $this->db->from($this->tbl_name);
        $this->db->join("sys_users as su", "su.user_id = umkm.user_id", "left");
        $this->db->join("ref_provinsi as rp", "rp.id_provinsi = umkm.id_provinsi", "left");
        $this->db->join("ref_kabupaten as rk", "rk.id_kabupaten = umkm.id_kabupaten", "left");
        $this->db->join("ref_kecamatan as rkec", "rkec.id_kecamatan = umkm.id_kecamatan", "left");
        $this->db->join("ref_kelurahan as rkel", "rkel.id_kelurahan = umkm.id_kelurahan", "left");
    ...
    {
        $this->db->select("tbb.name , tbb.bulan , tbb.desc , tbb.jml, ss.value_setting, tkbb.name as kat_bb ");
        $this->db->from("tbl_bahan_baku tbb");
        // $this->db->join("tbl_umkm as su", "su.user_id = tbb.user_id", "left");
        $this->db->join("tbl_kat_bahan_baku as tkbb", "tkbb.id_kat_bahan_baku = tbb.id_kat_bahan_baku", "left");
        $this->db->join("sys_settings as ss", "ss.id_setting = tbb.id_satuan_bb", "left");

        $this->db->where("tbb.user_id = $id AND ss.category_setting='satuan_bb'");
        $this->db->order_by("tbb.bulan", 'desc');
        $query = $this->db->get();
        // die($this->db->last_query());
    ...
    public function getDataProduct($id) {
        $this->db->select("tbp.id_product, tbp.user_id, tbp.name, tbp.price, tbp.id_kat_prod, tbp.desc, tbp.img");
        $this->db->from("tbl_product tbp");
        $this->db->join("tbl_umkm as tbu", "tbu.user_id = tbp.user_id", "left");

        $this->db->where("tbu.id_umkm", $id);
        $query = $this->db->get();
        return ($query->num_rows() > 0) ? $query->result_array() : [];
    }
    ...
    public function exportToExcel($xls_data)
    {
        ...
        $arr_col = array("No", 'NIK', 'Nama Umkm', 'No. BPJS', 'Tempat Lahir', 'Tgl Lahir', 'Jenis Kelamin', 'Kabupaten', 'Kecamatan', 'Kelurahan', 'Alamat', 'No. HP', 'Pendidikan', 'Pekerjaan', 'Status Kawin', 'Gol. Darah', 'Create At', 'Update At');
