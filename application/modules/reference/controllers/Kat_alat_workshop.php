...
class Kat_alat_workshop extends CI_Controller
{
    private $menu = 'Home';
    private $idx_base_url = 'kat_alat_workshop/ajaxPaginationData';
    private $model_name = 'kat_alat_workshop_model';
    private $page_topic = 'kat_alat_workshop';
    private $controller = 'reference/kat_alat_workshop';
    private $views_folder_group = 'kat_alat_workshop/';
    private $user_id = '';
    private $key_edit = 'id_kat_alat_workshop';
    private $search_place_holder = 'Untuk pencarian data, ketik Nama Kategori Alat Workshop, lalu tekan enter';
    private $col_title = array('Kategori Alat Workshop', 'Create At', 'Update At');
    private $list_field = array('name', 'create_at', 'update_at');
    private $page_title = 'Data Kategori Alat Workshop';
    private $table_width = '100';
    private $tblname = 'tbl_kat_alat_workshop';
    private $export = true;
    ...
    public function index()
    {
        ...
        $list_data = generateGridElement($this->col_title, $this->list_field, $data_src, $this->key_edit,  $this->table_width, $this->tblname, '', '', 'edit', $access, '', $access, $component, '', '', '', $this->export);
    ...
    public function displayInputForm()
    {
        $data = array(
            'controller' => $this->controller,
            'form_input_title' => 'Input Data kategori Alat Workshop',
            'root' => $this->menu,
        );
    ...
    public function displayImportForm()
    {
        $data = array(
            'controller' => $this->controller,
            'form_input_title' => 'Import Data kategori Alat Workshop',
            'root' => $this->menu,
        );
        $this->load->view($this->views_folder_group . $this->page_topic . '_import', $data);
    }
    ...
    public function saveData()
    {
        ...
        if (is_data_exist('tbl_kat_alat_workshop', 'name', "name = '$validate_value'")) {
            $arr_response = array(
                'error' => 'true',
                'msg' => 'Nama kategori Alat Workshop <span class="badge badge-warning">' . $validate_value . '</span>' . MSG_DATA_ALREADY_EXIST,
            );
    ...
    public function editData()
    {
        ...
        $data = array(
            ...
            'form_input_title' => 'Edit Data Kategori Alat Workshop',
            'root' => $this->menu,
            'dt_init' => $init_data,
        );
    ...
    public function importData()
    {
        ...
        if (!$this->upload->do_upload('file')){
            ...
            $arr_response = array(
                'error' =>'true',
                'msg' => 'Upload Error, Harap Periksa kembali File',
            );
            // echo json_encode($arr_response);
        }else{
            // echo 2;
            ...
            $import = $this->$nm_model->importexcel($path,$this->user_id);
            $arr_response = array(
                'error' => 'false',
                'msg' => 'Upload Sukses, Data masuk '.$import.' row(s)',
                'path' => $_FILES["file"]["tmp_name"]
            );
            // echo json_encode($arr_response);
        }
