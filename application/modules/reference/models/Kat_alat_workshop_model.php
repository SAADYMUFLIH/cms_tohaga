...
class Kat_alat_workshop_model extends CI_Model
{
    private $selected_field = "tbl_kat_alat_workshop.*";
    private $tbl_name = "tbl_kat_alat_workshop";
    private $field_search = array("name");
    private $field_id = "id_kat_alat_workshop";
    ...
    public function exportToExcel($xls_data)
    {
        ...
        $arr_col = array('No', 'Nama Kategori Alat Workshop', 'Create At', 'Update At');
        ...
        $objPHPExcel->getActiveSheet(0)->setTitle('Data Kategori Alat Workshop');

        $filename = "Datakat_alat_workshop";
    ...
    public function getRwByIdKel($id_kat_alat_workshop)
    {
        $this->db->select("id_rw as kode, no_rw as nama");
        $this->db->from("tbl_rw");
        $this->db->where("id_kat_alat_workshop = '$id_kat_alat_workshop'");
        $query = $this->db->get();
        $arr = [];
        ...
        foreach($object->getWorksheetIterator() as $worksheet){
            ...
            for($row=2; $row<=$highestRow; $row++){
                ...
                if($this->checkifexist($kategori)==0){
                    $insert = $this->db->query("INSERT IGNORE INTO `tbl_kat_alat_workshop` (
                        `name`,
                        `create_by`,
                        `create_at`
                        )Values(
    ...
    {
        $query = $this->db->query("select * from tbl_kat_alat_workshop where name = '$name' limit 1");
