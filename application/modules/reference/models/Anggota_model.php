...
{
    private $selected_field = "tbl_anggota.*";
    private $tbl_name = "tbl_anggota";
    private $field_search = array("name");
    private $field_id = "id_anggota";
    ...
    public function exportToExcel($xls_data)
    {
        ...
        $arr_col = array('No', 'Nama Anggota', 'Create At', 'Update At');
        ...
        $objPHPExcel->getActiveSheet(0)->setTitle('Data Anggota Dewan');

        $filename = "Dataanggota";
    ...
    public function getRwByIdKel($id_anggota)
    {
        ...
        foreach($object->getWorksheetIterator() as $worksheet){
            ...
            for($row=2; $row<=$highestRow; $row++){
                ...
                if($this->checkifexist($kategori)==0){
                    $insert = $this->db->query("INSERT IGNORE INTO `tbl_anggota` (
                        `name`,
                        `create_by`,
                        `create_at`
                        )Values(
    ...
    {
        $query = $this->db->query("select * from tbl_anggota where name = '$name' limit 1");
