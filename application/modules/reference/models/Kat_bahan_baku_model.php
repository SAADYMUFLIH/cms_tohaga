...
class Kat_bahan_baku_model extends CI_Model
{
    private $selected_field = "tbl_kat_bahan_baku.*";
    private $tbl_name = "tbl_kat_bahan_baku";
    private $field_search = array("name");
    private $field_id = "id_kat_bahan_baku";
    ...
    public function exportToExcel($xls_data)
    {
        ...
        $arr_col = array('No', 'Nama Kategori Bahan Baku', 'Create At', 'Update At');
    ...
    public function getRwByIdKel($id_kat_bahan_baku)
    {
        $this->db->select("id_rw as kode, no_rw as nama");
        $this->db->from("tbl_rw");
        $this->db->where("id_kat_bahan_baku = '$id_kat_bahan_baku'");
        $query = $this->db->get();
        $arr = [];
        ...
        foreach($object->getWorksheetIterator() as $worksheet){
            ...
            for($row=2; $row<=$highestRow; $row++){
                ...
                if($this->checkifexist($kategori)==0){
                    $insert = $this->db->query("INSERT IGNORE INTO `tbl_kat_bahan_baku` (
                        `name`,
                        `create_by`,
                        `create_at`
                        )Values(
    ...
    {
        $query = $this->db->query("select * from tbl_kat_bahan_baku where name = '$name' limit 1");
