...
class Kat_dewan_model extends CI_Model
{
    private $selected_field = "tbl_kat_dewan.*";
    private $tbl_name = "tbl_kat_dewan";
    private $field_search = array("name");
    private $field_id = "id_kat_dewan";
    ...
    public function exportToExcel($xls_data)
    {
        ...
        $arr_col = array('No', 'Kategori Anggota Dewan', 'Create At', 'Update At');
        ...
        $objPHPExcel->getActiveSheet(0)->setTitle('Data Kategori Anggota Dewan');

        $filename = "Datakat_dewan";
    ...
    public function getRwByIdKel($id_kat_dewan)
    {
        $this->db->select("id_rw as kode, no_rw as nama");
        $this->db->from("tbl_rw");
        $this->db->where("id_kat_dewan = '$id_kat_dewan'");
        $query = $this->db->get();
        $arr = [];
        ...
        foreach($object->getWorksheetIterator() as $worksheet){
            ...
            for($row=2; $row<=$highestRow; $row++){
                ...
                if($this->checkifexist($kategori)==0){
                    $insert = $this->db->query("INSERT IGNORE INTO `tbl_kat_dewan` (
                        `name`,
                        `create_by`,
                        `create_at`
                        )Values(
    ...
    {
        $query = $this->db->query("select * from tbl_kat_dewan where name = '$name' limit 1");
